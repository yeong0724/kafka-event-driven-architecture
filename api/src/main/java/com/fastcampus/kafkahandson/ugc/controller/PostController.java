...
public class PostController {
    ...
    ResponseEntity<PostDto> createPost(@RequestBody PostCreateRequest postCreateRequest) {
        Post post = postCreateUsecase.create(
                new PostCreateUsecase.Request(
                        ...
                        postCreateRequest.getCategoryId()
                )
        );
    ...
    @PutMapping("/{postId}")
    ResponseEntity<PostDto> updatePost(@PathVariable("postId") Long id, @RequestBody PostUpdateRequest postUpdateRequest) {
        Post post = postUpdateUsecase.update(
                new PostUpdateUsecase.Request(
                        id,
                        postUpdateRequest.getTitle(),
                        postUpdateRequest.getContent(),
                        postUpdateRequest.getCategoryId()
                )
        );
